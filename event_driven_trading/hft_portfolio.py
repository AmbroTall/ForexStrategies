...
class PortfolioHFT(object):
    ...
    def construct_all_holdings(self):
        ...
        to determine when the time index will begin.
        """
        initial_holdings = {symbol: 0.0 for symbol in self.symbol_list}
        initial_holdings['datetime'] = self.start_date
        initial_holdings['cash'] = self.initial_capital
        initial_holdings['commission'] = 0.0
        initial_holdings['total'] = self.initial_capital
    ...
    def update_timeindex(self, event):
        ...
        # Update positions
        # ================
        positions_data = {'datetime': latest_datetime}
        for symbol in self.symbol_list:
            positions_data[symbol] = self.current_positions[symbol]
        self.all_positions.append(positions_data)

        # Update holdings
        # ===============
        holdings_data = {'datetime': latest_datetime,
        ...
        for symbol in self.symbol_list:
            ...
            holdings_data[symbol] = market_value
            holdings_data['total'] += market_value
    ...
    def update_holdings_from_fill(self, fill):
        ...
        fill_dir = 1 if fill.direction == 'BUY' else -1 if fill.direction == 'SELL' else 0
